...
class DocxSplitterV2:
    ...
    def extract_docx_by_multiple_ranges(self, ranges, output_path):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 识别目录区域的结束位置
            # 目录通常在文档开头，找到第一个不是 TOC 样式且不包含 TOC 字段的段落
            toc_region_end_idx = 0
            for i, para in enumerate(doc.paragraphs):
                ...
                if 'toc' not in style_name:
                    ...
                    # 如果既不是 TOC 样式，也不包含 TOC 字段，说明目录结束了
                    if not has_toc_field:
                        toc_region_end_idx = i
                        print(f"  检测到目录区域结束于段落 {i}")
                        break
            ...
            for element in list(body):
                ...
                if element.tag.endswith('p'):
                    ...
                    # --- 新增/修改：目录删除逻辑 ---
                    # 如果段落被选中（should_remove == False），但它在目录区域内，则进一步检查是否是目录相关内容
                    if not should_remove and para_count < toc_region_end_idx:
                        is_toc_item = False
                        ...
                        if para_count < len(para_styles):
                            ...
                            if 'toc' in style_name:
                                is_toc_item = True
                                print(f"  删除目录样式段落 (idx {para_count}) (在目录区域内): {style_name}")
                        ...
                        if not is_toc_item:
                            for child in element.iter():
                                ...
                                if child.tag == f'{w_namespace}instrText':
                                    if child.text and ('TOC' in child.text or 'HYPERLINK' in child.text):
                                        is_toc_item = True
                                        print(f"  删除TOC/HYPERLINK字段段落 (idx {para_count}) (在目录区域内)")
                                        break
            ...
            if len(ranges) > 1:
                ...
                last_heading_level = None  # 记录前一个标题的级别
                
                for i, para in enumerate(doc.paragraphs):
                    level = self.get_paragraph_style_level(para)
                    # 在1级、2级和3级标题前插入分页符，且不是第一个段落
                    if level is not None and level <= 3 and i > 0:
                        should_insert = True
                        
                        # 如果是2级标题，检查前一个标题是否是1级标题
                        # 如果是，则不插入分页符（因为它们属于同一章节）
                        if level == 2 and last_heading_level == 1:
                            should_insert = False
                        
                        # 如果是3级标题，检查前一个标题是否是1级或2级标题
                        # 如果是，则不插入分页符（因为它们属于同一章节）
                        if level == 3 and last_heading_level is not None and last_heading_level <= 2:
                            should_insert = False
                        ...
                        if should_insert:
                            ...
                            if parent is not None:
                                ...
                                parent.insert(parent.index(para_element), page_break_para)
                                inserted_count += 1
                                print(f"    - 在 '{para.text[:30]}...' 前插入分页符 (Level {level})")
                ...
                if inserted_count > 0:
                    print(f"  已插入 {inserted_count} 个分页符")
    ...
    def save_section_as_pdf(self, start_idx, end_idx, output_path):
        """保存段落范围为PDF"""
        # 在本地创建临时路径
        local_temp_dir = self.temp_workspace.name
        temp_docx = os.path.join(local_temp_dir, 'section_temp.docx')
        temp_pdf = os.path.join(local_temp_dir, 'section_temp.pdf')
        
        # 提取内容到临时docx
        self.extract_docx_by_paragraph_range(start_idx, end_idx, temp_docx)
        
        # 转换为PDF
        success = self.convert_docx_to_pdf(temp_docx, temp_pdf)
        
        # 如果转换成功,处理PDF
        if success and os.path.exists(temp_pdf):
            self.remove_blank_pages(temp_pdf)
            # 移动到最终位置
    ...
    def save_sections_as_pdf(self, ranges, output_path):
        ...
        # 在本地创建临时路径
        local_temp_dir = self.temp_workspace.name
        temp_docx = os.path.join(local_temp_dir, 'combined_temp.docx')
        temp_pdf = os.path.join(local_temp_dir, 'combined_temp.pdf')
