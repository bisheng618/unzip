...
def get_libreoffice_path():
    """
    根据操作系统自动检测LibreOffice可执行文件路径
    支持 macOS 和 Linux (包括 CentOS 7.9)
    """
    ...
    elif system == "Linux":  # Linux (包括 CentOS 7.9)
        paths = [
            '/usr/bin/soffice',
            '/usr/bin/libreoffice',
    ...
    # 如果没有找到，尝试使用 which 命令 (仅限 Unix-like 系统)
...
class DocxSplitterV2:
    def __init__(self, source_file, output_dir):
        ...
        # 获取LibreOffice路径
        self.libreoffice_path = get_libreoffice_path()
    ...
    def convert_docx_to_pdf(self, docx_path, pdf_path):
        ...
        try:
            if not self.libreoffice_path:
                print(f"✗ 转换失败: 未找到LibreOffice可执行文件")
                return False
    ...
    def split_patents(self):
        ...
        for i in range(start_search_idx, len(self.doc.paragraphs)):
            para = self.doc.paragraphs[i]
            text = para.text.strip()
            if "专利" in text and ("数量" in text or "情况" in text or "列表" in text) and self.is_valid_heading(para):
                parent_idx = i
                self.debug_print_para_info(i, "Found Patent Start")
                break
        ...
        # 找到专利章节的结束位置（下一个2级或更高级别的标题）
        end_idx = None
        for i in range(parent_idx + 1, len(self.doc.paragraphs)):
            level = self.get_paragraph_style_level(self.doc.paragraphs[i])
            if level is not None and level <= 2:
                end_idx = i
                self.debug_print_para_info(i, "Found Patent End")
                break
...
def main():
    ...
    if not libreoffice_path:
        ...
        if system == "Linux":
            ...
            print("或者:")
            print("  sudo yum install libreoffice-headless")
        elif system == "Darwin":
            print("\nmacOS 安装命令:")
            print("  brew install --cask libreoffice")
        return
    
    print(f"找到LibreOffice: {libreoffice_path}")
